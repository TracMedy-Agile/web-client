  ...
  return (
    <SectionFrame icon={<FlaskConical className="h-4 w-4" />} title="Laboratory & Diagnostic Tests" subtitle="Recommended tests, imaging and scans for this care plan">
      <div className="space-y-4">{items.map((item, index) => <div key={item.id} className="rounded-xl bg-muted/60 p-3">
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-[120px_1fr_110px_auto]">
          <label className="space-y-1">
            <span className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">Type</span>
            <select aria-label="Test type" className={fieldClass}><option>Laboratory Test</option><option>Diagnostic Test</option><option>Imaging</option></select>
          </label>
          <label className="space-y-1">
