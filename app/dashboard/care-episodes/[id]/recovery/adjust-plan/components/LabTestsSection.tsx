  ...
  return (
    <SectionFrame icon={<FlaskConical className="h-4 w-4" />} title="Laboratory & Diagnostic Tests" subtitle="Recommended tests, imaging and scans for this care plan">
      <div className="space-y-4">{items.map((item, index) => <div key={item.id} className="rounded-xl bg-[#F1F3F5] p-3">
        ...
        <div className="mt-2 grid gap-2 sm:grid-cols-[1fr_150px]"><input aria-label="Purpose" className={fieldClass} value={item.purpose} onChange={(event) => update(index, { purpose: event.target.value })} /><label><span className="mb-1 block text-[8px] font-bold uppercase text-[#7D8BA0]">Expected date</span><input type="date" aria-label="Expected date" className={fieldClass} value={item.date} onChange={(event) => update(index, { date: event.target.value })} /></label></div>
      </div>)}</div>
      <AddRowButton onClick={() => setAdding(true)}>Add test recommendation</AddRowButton>
      <AddTaskModal kind="lab" open={adding} onClose={() => setAdding(false)} onAdd={(task) => onChange([...items, { id: crypto.randomUUID(), name: task.title, priority: task.priority, purpose: task.summary || task.description, date: task.date }])} />
