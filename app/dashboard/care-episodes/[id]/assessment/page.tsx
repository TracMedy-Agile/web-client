...
const OUTCOME_DOT_CLASSNAME: Record<AssessmentOutcome, string> = {
  Improving: "bg-[#10B981]",
  Stable: "bg-[#023E8A]",
  "Delayed Recovery": "bg-[#F59E0B]",
  Deteriorating: "bg-[#EF4444]",
  Resolved: "bg-[#10B981]",
  "No Escalation": "bg-[#10B981]",
  Escalated: "bg-[#EF4444]",
  "Care Plan Adjustment": "bg-[#F59E0B]",
};
...
function fieldClass(extra = "") {
  return cn(
    "h-11 w-full rounded-lg border border-transparent bg-[#F1F3F5] px-3.5 text-sm font-medium text-[#172033] outline-none placeholder:text-[#7689A5] focus-visible:border-[#74A9E5] focus-visible:bg-white focus-visible:ring-2 focus-visible:ring-[#0B5CAB]/10",
    extra,
  );
}
      ...
      <CardContent className="p-5 sm:p-6">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-md bg-[#E7F2FF] text-[#1769C2]">{icon}</span>
            <h2 className="text-sm font-bold text-[#111827] sm:text-base">{title}</h2>
          </div>
          <button
            type="button"
            onClick={() => setExpanded((current) => !current)}
            className="flex items-center gap-2 rounded-full bg-[#DFFBF0] px-3 py-1.5 text-xs font-bold text-[#10B981]"
          >
            {confidencePercent}% <span className="hidden sm:inline">CONFIDENCE</span>
...
function SourceTag({ label }: { label: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-[#BEE8D2] px-3 py-1 text-xs font-bold text-[#10B981]">
...
export default function ClinicalAssessmentWorkspacePage() {
  ...
  useEffect(() => {
    if (!episodeId) return;
    let ignore = false;
    Promise.allSettled([getCareEpisodeById(episodeId), getAssessmentWorkspace(episodeId)]).then(([episodeResult, workspaceResult]) => {
      if (ignore) return;
      if (episodeResult.status === "fulfilled") setEpisode(episodeResult.value);
      if (workspaceResult.status === "fulfilled") {
        const entry = workspaceResult.value;
        setWorkspace(entry);
        setOutcome("Delayed Recovery");
        setRecommendedActions(["Adjust Care Plan"]);
    ...
    <div className="mx-auto max-w-[1200px] space-y-6 pb-8">
      ...
      <div>
        <div className="flex flex-wrap items-center gap-3">
          <h1 className="text-xl font-bold text-[#111827] md:text-2xl">Clinical Assessment Workspace</h1>
          <span className="inline-flex items-center gap-1.5 rounded-full bg-[#E7F2FF] px-3 py-1 text-xs font-bold text-[#1769C2]">
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-5">
          <IntelligenceCard icon={<Stethoscope className="h-5 w-5" />} title="A · Clinical Status Intelligence" confidencePercent={workspace.clinicalStatus.confidencePercent}>
            <div className="rounded-xl border border-[#B9D8FF] bg-[#F0F6FF] p-4">
              <p className="flex items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-[0.06em] text-[#1769C2]">
                <Sparkles className="h-3.5 w-3.5" />
                AI Summary
              </p>
              <p className="mt-2 text-sm leading-6 text-[#344054]">{workspace.clinicalStatus.summary}</p>
            </div>
            <p className="mb-2 mt-4 text-xs font-extrabold uppercase tracking-[0.06em] text-[#71809B]">Sources</p>
          ...
          <IntelligenceCard icon={<ClipboardList className="h-5 w-5" />} title="B · Care Plan Adherence Intelligence" confidencePercent={workspace.carePlanAdherence.confidencePercent}>
            <p className="text-sm font-medium text-[#71809B]">{workspace.carePlanAdherence.summary}</p>
            <div className="mt-3 flex flex-wrap items-center gap-4">
              <span className="text-4xl font-extrabold text-[#111827]">{workspace.carePlanAdherence.adherencePercent}%</span>
              <div className="h-2 min-w-40 flex-1 overflow-hidden rounded-full bg-[#E5E7EB]">
                <div className="h-full rounded-full bg-[#F59E0B]" style={{ width: `${workspace.carePlanAdherence.adherencePercent}%` }} />
              </div>
              <span className="inline-flex shrink-0 rounded-full bg-[#FFECEC] px-3 py-1 text-xs font-bold text-[#EF4444]">
                {workspace.carePlanAdherence.trendLabel} ({workspace.carePlanAdherence.trendDeltaPercent}%)
              </span>
            </div>
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              {workspace.carePlanAdherence.breakdown.map((item) => (
                <span key={item.label} className={cn("inline-flex items-center gap-2 text-sm font-medium", item.positive ? "text-[#10B981]" : "text-[#EF4444]")}>
                  {item.positive ? <CircleCheck className="h-4 w-4 shrink-0" /> : <XIcon className="h-4 w-4 shrink-0 rounded-full bg-[#FFECEC] p-0.5" />}
                  {item.label}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-[#DDE3EC] px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.06em] text-[#71809B]">
          ...
          <Card className="rounded-xl border-[#DDE3EC] bg-white shadow-sm">
            <CardContent className="p-5 sm:p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-sm font-bold text-[#111827] sm:text-base">Clinician Findings</h2>
                  <p className="text-xs font-medium text-[#71809B]">Your assessment based on the evidence above</p>
                </div>
              </div>
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <label className="block">
                  <span className="mb-2 block text-[10px] font-extrabold uppercase tracking-[0.04em] text-[#182132]">Symptom Status</span>
                  <input value={symptomStatus} onChange={(event) => setSymptomStatus(event.target.value)} className={fieldClass()} />
                </label>
                <label className="block">
                  <span className="mb-2 block text-[10px] font-extrabold uppercase tracking-[0.04em] text-[#182132]">Treatment Response</span>
                  <input value={treatmentResponse} onChange={(event) => setTreatmentResponse(event.target.value)} className={fieldClass()} />
                </label>
              </div>
              <label className="mt-4 block">
                <span className="mb-2 block text-[10px] font-extrabold uppercase tracking-[0.04em] text-[#182132]">Key Observation</span>
                <input value={keyObservation} onChange={(event) => setKeyObservation(event.target.value)} className={fieldClass()} />
              </label>
              <label className="mt-4 block">
                <span className="mb-2 block text-[10px] font-extrabold uppercase tracking-[0.04em] text-[#182132]">Clinical Assessment Notes</span>
                <Textarea
                  value={clinicalNotes}
        ...
        <aside className="space-y-5 lg:sticky lg:top-4">
          <Card className="rounded-xl border-[#DDE3EC] bg-white shadow-sm">
            <CardContent className="p-5">
              <h2 className="text-xs font-extrabold uppercase tracking-[0.06em] text-[#71809B]">Assessment Outcome</h2>
                  ...
                  return (
                    <label
                      ...
                      className={cn(
                        ...
                        selected ? "border-[#F59E0B] bg-[#FFF8EC] text-[#B45309]" : "border-[#E5E7EB] text-[#344054]",
                      )}
                    >
                      <span className="flex items-center gap-3">
                        <RadioGroupItem value={option} className={selected ? "border-[#F59E0B] text-[#F59E0B]" : ""} />
                        {option}
                      </span>
          ...
          <Card className="rounded-xl border-[#DDE3EC] bg-white shadow-sm">
            <CardContent className="p-5">
              <h2 className="text-xs font-extrabold uppercase tracking-[0.06em] text-[#71809B]">Recommended Actions</h2>
                  ...
                  return (
                    <label
                      ...
                      className={cn(
                        ...
                        checked ? "border-[#023E8A] text-[#023E8A]" : "border-[#E5E7EB] text-[#344054]",
                      )}
                    >
          ...
          <Button
            ...
            onClick={handleSave}
            disabled={isSaving}
            className="h-12 w-full rounded-xl bg-[#023E8A] text-sm font-bold text-white hover:bg-[#023575]"
          >
            {isSaving ? (
              <>
          ...
          <Button
            ...
            variant="outline"
            disabled={isSaving}
            className="h-11 w-full rounded-xl border-[#DDE3EC] bg-white text-sm font-bold text-[#111827] hover:bg-[#F8FAFC]"
          >
            <Link href={`/dashboard/care-episodes/${episodeId}`}>Cancel</Link>
          </Button>
          <p className="text-center text-xs font-medium text-[#71809B]">Dr. Emeka Nwosu · auto-timestamped</p>
        </aside>
      </div>
    </div>
  );
